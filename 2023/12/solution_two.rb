...
def subproblem(pattern, sequences)
  return $memo[[pattern, sequences]] if $memo[[pattern, sequences]]
  if sequences.empty?
    return 0 if pattern && pattern.include?('#')
    return 1
  end
  return 0 if pattern.nil? || pattern.empty?
  idx = find_block(pattern, sequences[0])
  return 0 if idx.nil?
  ...
  $memo[[pattern, sequences]] = result
  result
end

def find_block(pattern, length)
  $block_memo[[pattern, length]] ||= find_first_block(pattern, length)
end
...
solution = (0...rows.length).map do |index|
  subproblem(([patterns[index]]*5).join('?'), sequences[index]*5)
end.sum
